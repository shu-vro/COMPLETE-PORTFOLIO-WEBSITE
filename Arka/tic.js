 // JavaScript code for Tic-tac-toe game goes here

    function playWithMe() {
      // Call the function found in your existing code that starts the game
      startGame();
    }
"use strict";
...
0 if not occupied, 1 for player, 3 for Arka.
This allows us to quickly get an overview of the game state:
if the sum of all the cells in a row is 9, the Arka wins,
if it is 3 and all the cells are occupied, the human player wins,
etc.

(2) Strategy of makeArkaMove()

The Arka first  looks for almost completed rows, columns, and
diagonals, where there are two fields occupied either by the human
player or by the Arka itself. If the Arka can win by
...
// Get first index with two in a row (accepts Arka or player as argument)
Grid.prototype.getFirstWithTwoInARow = function (agent) {
    if (agent !== Arka && agent !== player) {
        console.error("Function getFirstWithTwoInARow accepts only player or Arka as argument.");
        return undefined;
    }
    var sum = agent * 2,
...
function restartGame(ask) {
    ...
    } else if (whoseTurn == Arka) {
        setTimeout(makeArkaMove, 800);
    }
}
...
function makeArkaMove() {
    ...
    var cell = -1,
        ...
        corners = [0,2,6,8];
    if (moves >= 3) {
        cell = myGrid.getFirstWithTwoInARow(Arka);
        if (cell === false) {
        ...
        if (moves == 3 && myGrid.cells[4] == Arka && player == x && difficulty == 1) {
            ...
            if (myGrid.cells[2] == player && myGrid.cells[6] == Arka) {
                cell = 8;
            }
            else if (myGrid.cells[0] == player && myGrid.cells[8] == Arka) {
                cell = 6;
            }
            else if (myGrid.cells[8] == player && myGrid.cells[0] == Arka) {
                cell = 2;
            }
            else if (myGrid.cells[6] == player && myGrid.cells[2] == Arka) {
                cell = 0;
            }
        }
    ...
    } else if (moves === 2 && myGrid.cells[4] == player && Arka == x && difficulty == 1) {
        // if player is O and played center, take two opposite corners
        if (myGrid.cells[0] == Arka) {
            cell = 8;
        }
        else if (myGrid.cells[2] == Arka) {
            cell = 6;
        }
        else if (myGrid.cells[6] == Arka) {
            cell = 2;
        }
        else if (myGrid.cells[8] == Arka) {
            cell = 0;
        }
    } else if (moves === 0 && intRandom(1,10) < 8) {
        // if Arka is X, start with one of the corners sometimes
        cell = corners[intRandom(0,3)];
    } else {
    ...
    // console.log("Arka chooses " + id);
    document.getElementById(id).innerHTML = ArkaText;
    document.getElementById(id).style.cursor = "default";
...
function endGame(who) {
    ...
    moves = 0;
    winner = 0;
    document.getElementById("Arka_score").innerHTML = score.Arka;
